...
function validateForm() {
    ...
    let NameErr = EmailErr = MobileErr = CountryErr = PincodeErr = true;
    ...
    if(pincode == "") {
        ...
        if(regex.test(pincode) === false) {
            printError("pincodeErr", "Please enter a valid pin code");
            var elem = document.getElementById("pincode");
            elem.classList.add("input-2");
            elem.classList.remove("input-1");
        } else{
    ...
    if((NameErr || EmailErr || MobileErr || CountryErr || PincodeErr) == true) {
       return false;
    } else{
        return true;
    }
